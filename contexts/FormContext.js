...
function reducer(fields, action) {
  switch (action.type) {
    case ACTIONS.UPDATE:
      return { ...fields, [action.key]: action.value };
    case ACTIONS.CHANGE_MODE:
      return { ...fields, mode: action.payload };
...
const options = {
  ...
  department: [
    ...
    { label: "Operations Department", value: 2 },
    { label: "Finance Department", value: 3 },
    { label: "Human Resource Department", value: 5 },
    { label: "Purchase Department", value: 6 },
    { label: "Sales Department", value: 7 },
  ],
  position: [
    "accountant",
    "actor",
...
function parseArrays(array) {
  return array.map((position, key) => {
    return { label: position, value: key };
  });
}

const categories = parseArrays(options.categories);

const positions = parseArrays(options.position);

const leaves = parseArrays(options.leaveType)
...
function FormContextProvider({ children }) {
  ...
  const addJobForm = [
    ...
    },
    {
      ...
      source: "internal",
      label: "Job Modality",
      value: fields.jobModality,
      options: options.jobModality,
    },
    {
      type: "select",
      source: "internal",
  ...
  const storeInfo = (event) => {
    ...
    dispatch({ type: ACTIONS.UPDATE, key: [name], value });
  };
  ...
  const resetFields = () => {
    dispatch({ type: ACTIONS.RESET_FIELDS });
  };

  // Exposes State, Dispatch and Actions
  const config = {
    fields,
    dispatch,
    ACTIONS,
  ...
  return (
    <FormContext.Provider value={{ ...exposed }}>
      {children}
